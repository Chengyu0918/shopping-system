...
<head>
    <script>
        ...
        $(function () {
            ...
            $("#btn-ok").click(() => {
                $('#modalDiv').modal("hide");
                let {userId, state} = JSON.parse(localStorage.getItem("changeUser"));
                if (state == "del") {
                    fetch("admin/user/del/" + userId).then(res => res.json()).then(res => {
                        let {status} = res;
                        if (status > 0) {
                            alert("删除成功")
                            //清除数据
                ...
                success: function (data) {
                    ...
                    if (data.userList.length > 0) {
                        for (var i = 0; i < data.userList.length; i++) {
                            ...
                            var user_level;
                            if (data.userList[i].user_level === 1) {
                                user_level = "普通会员";
                            } else if (data.userList[i].user_level === 2) {
                                user_level = "白银会员";
                            } else if (data.userList[i].user_level === 3) {
                                user_level = "黄金会员";
                            }
                            ...
                            tbody.append("" +
                                "<tr>" +
                                "<td><input type='checkbox' class='cbx_select' id='cbx_user_select_" + user_id + "'>" +
                                "<label for='cbx_user_select_" + user_id + "'></label>" +
                                "</td>" +
                                "<td title='" + user_name + "'>" + user_name + "</td>" +
                                "<td title='" + user_nickname + "'>" + user_nickname + "</td>" +
                                "<td title='" + user_realname + "'>" + user_realname + "</td>" +
                                "<td title='" + user_level + "'>" + user_level + "</td>" +
                                "<td title='" + user_birthday + "'>" + user_birthday + "</td>" +
                                "<td title='" + gender + "'>" + gender + "" +
                                "</td>" +
                                "<td>" +
                                "<span class='td_warn' title='查看用户详情'>" +
                                "<a href='javascript:void(0);' onclick='getChildPage(this)'>详情</a>" +
                                "</span>" +
                                " <span class='td_special' title='删除用户'>" +
                                "<a href='javascript:void(0);' onclick='delChildPage(this)'>删除</a>" +
                                "</span>" +
                                " <span class='td_success' title='重置密码'>" +
                                "<a href='javascript:void(0);' onclick='resetPwd(this)'>重置密码</a>" +
                                "</span>" +
                                "</td>" +
                                "<td hidden  class='user_id'>" + user_id + "</td>" +
                                "</tr>");
                        }
                        //绑定事件
        ...
        function delChildPage(obj) {
            let name = $(obj).parents("tr").find(".user_name").text();
            let userId = $(obj).parents("tr").find(".user_id").text();
            console.log($(obj).parents("tr").find(".user_id").text());
            // let del = confirm(`请确认是否删除` + name + "？");
            $(".modal-body").text(`请确认是否删除` + name + "？")
            $('#modalDiv').modal();
            let changeUser = {userId, state: "del"};
        ...
        function resetPwd(obj) {
            ...
            let userId = $(obj).parents("tr").find(".user_id").text();
            console.log($(obj).parents("tr").find(".user_id").text());
            $(".modal-body").text(`请确认是否初始化密码？`)
            $('#modalDiv').modal();
            let changeUser = {userId, state: "reset"};
            localStorage.setItem("changeUser", JSON.stringify(changeUser));
        }
...
<div class="table_normal_div">
    <table class="table_normal" id="table_user_list">
        ...
        <tr>
            ...
            <th class="data_info" data-sort="asc" data-name="user_level">
                <span>级别</span>
                <span class="orderByDesc"></span>
                <span class="orderByAsc orderBySelect"></span>
            </th>
        ...
        <c:forEach items="${requestScope.userList}" var="user">
            <tr>
                <td><input type="checkbox" class="cbx_select" id="cbx_user_select_${user.user_id}"><label
                        for="cbx_user_select_${user.user_id}"></label></td>
                <td class="user_name" title="${user.user_name}">${user.user_name}</td>
                <td title="${user.user_nickname}">${user.user_nickname}</td>
                <td title="${user.user_realname}">${user.user_realname}</td>
                <td>
                    <c:choose>
                        <c:when test="${user.user_level==1}">普通会员</c:when>
                        <c:when test="${user.user_level==2}">白银会员</c:when>
                        <c:when test="${user.user_level==3}">黄金会员</c:when>
                    </c:choose>
                </td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn btn-primary" name="" id="btn-ok">确定</button>
