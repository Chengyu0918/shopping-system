...
public class UserController extends BaseController {
    ...
    @ResponseBody
    @Transactional
    @GetMapping(value = "admin/user/del/{userId}", produces = "application/json;charset=UTF-8")
    public Map delUserBySearch(@PathVariable Integer userId) {
        Map<String, Object> map = new HashMap<>();
        System.out.println(userId);
        // 删除评论表
        reviewMapper.delByUserId(userId);

        // 删除订单
        productOrderMapper.delByUserId(userId);

        // 删除订单信息
        productOrderItemMapper.delByUserId(userId);
        // 删除用户表
        int status = userService.del(userId);
    ...
    // 用户密码重置
    @ResponseBody
    @GetMapping(value = "admin/user/reset/{userId}", produces = "application/json;charset=UTF-8")
    public Map resetUser(@PathVariable Integer userId) {
        Map<String, Object> map = new HashMap<>();
        System.out.println(userId);
        // 初始化密码为123456
        int status = userService.updatePwd(userId);
        map.put("status", status);
        return map;
    }
